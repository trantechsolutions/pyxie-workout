...
describe('workoutSlice — preview + intensity/complexity', () => {
  ...
  it('setIntensity stores choice and resets previewList', () => {
    usePyxie.setState((s) => ({ ui: { ...s.ui, previewList: [{ name: 'x', cue: 'y', form: 'test form', movementPattern: 'push' as const }] } }));
  ...
  it('buildPreview is a no-op when previewList already exists', () => {
    const list = [{ name: 'x', cue: 'y', form: 'test form', movementPattern: 'push' as const }];
  ...
  it('rerollPreview always rewrites the list', () => {
    const stale = [{ name: 'x', cue: 'y', form: 'test form', movementPattern: 'push' as const }];
...
describe('workoutSlice — start/skip/finish', () => {
  ...
  it('startWorkout builds segments from the preview when present', () => {
    const preview = [
      { name: 'Push-ups', cue: 'a', form: 'test form', movementPattern: 'push' as const },
      { name: 'Squats', cue: 'b', form: 'test form', movementPattern: 'push' as const },
    ];
  ...
  it('finishWorkout(false) clears workout state and previewList without rewarding xp', () => {
    ...
    usePyxie.setState({
      pet,
      ui: { ...usePyxie.getState().ui, workoutActive: makeActiveWorkout(), previewList: [{ name: 'x', cue: 'y', form: 'test form', movementPattern: 'push' as const }] },
    });
...
describe('uiSlice', () => {
  ...
  it('setTab updates tab and clears previewList', () => {
    usePyxie.setState((s) => ({ ui: { ...s.ui, previewList: [{ name: 'a', cue: 'b', form: 'test form', movementPattern: 'push' as const }] } }));
